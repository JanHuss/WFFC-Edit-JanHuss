...
void Game::Update(DX::StepTimer const& timer)
{
	...
	m_camera.ProcessMouseMovement(mouseState, deltaTime, firstRelativeFrame);
...
void Game::Render()
{
	...
	m_sprites->Begin();
	WCHAR   Buffer[256];
	std::wstring camPos = L"CamPosX: " + std::to_wstring(m_camera.GetPosition().x) + L" CamPosY: " + std::to_wstring(m_camera.GetPosition().y) + L" CamPosZ: " + std::to_wstring(m_camera.GetPosition().z);
	m_font->DrawString(m_sprites.get(), camPos.c_str(), XMFLOAT2(10, 10), Colors::Yellow);
	std::wstring camOr = L"CamOrX: " + std::to_wstring(m_camera.GetOrientation().x) + L" CamOrY: " + std::to_wstring(m_camera.GetOrientation().y);
	m_font->DrawString(m_sprites.get(), camOr.c_str(), XMFLOAT2(10, 30), Colors::Yellow);
	m_sprites->End();
...
void Game::setDialogBoxOpen(bool dBO)
{
	isDialogBox = dBO;
}

bool Game::getDialogBoxOpen()
{
	return isDialogBox;
}
